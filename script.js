document.addEventListener('DOMContentLoaded', () => {
    ...
    function generateCertifications() {
        ...
        certificationsGrid.innerHTML = certifications.map((cert, index) => `
            <div class="card project-card certification-card travel-card" data-category="${cert.category}">
                ...
                <div class="travel-overlay">
                    <h3 class="travel-title">${cert.title}</h3>
                    <div class="travel-sub">${cert.organisme} &nbsp;·&nbsp; ${cert.date}</div>
                    <a href="${cert.link}" class="btn-view-cert" data-index="${index}" aria-label="Voir le certificat ${cert.title}">
                        Voir le certificat <span class="travel-arrow">›</span>
                    </a>
                </div>
            </div>
        `).join('');
    ...
    function setupGoogleAnalyticsTracking() {
        ...
        projectBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                // Récupère le nom du projet via aria-label ou dataset pour un suivi précis
                const projectName = btn.getAttribute('aria-label') || btn.dataset.project || 'Projet inconnu';
                if (typeof gtag === 'function') {
        ...
        certBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                const certName = btn.getAttribute('aria-label') || 'Certificat inconnu';
                if (typeof gtag === 'function') {
